...
**After:**
> Her views have been cited in The New York Times, the BBC, the Financial Times and The Hindu. She has over 500,000 social media followers.

The rewrite drops the padding (*maintains an active presence*) and keeps every fact, including the
count and all four outlets. Cutting outlets or the number would be a claim loss, which the rewrite
process defines as an error. Name-dropping is fixed by removing the rhetoric of importance, not the
evidence.
...
**Keep these three.** The rule targets the dash as loose prose punctuation, not every occurrence of
the character: numeric ranges (`1990–2005`, `10–20 ms`, where a hyphen is a downgrade); dashes the
target language's grammar requires (see [Language scope](#language-scope)); and dashes inside
quotations, titles and proper names, where you are reproducing someone else's text.

Before returning the rewrite, search for `—` and `–`. Remove each one used as prose punctuation
unless the writer's sample uses that mark; in that case, match the sample's rate.
...
### 19. Quotation marks
**Problem:** ChatGPT often uses English curly quotes (“...”) where the writer or target format uses
something else.
**Rule:** Normalize to what the target language and format use, not to ASCII: straight quotes for
English prose and code, «ёлочки» with nested „лапки" for Russian, « » with spaces for French, „ "
for German. Converting «» to `"` is a downgrade to typewriter register, not a fix.
**Before:**
...
3. Check claim preservation, and do it mechanically rather than by impression:
   - List every fact, name, number, date, quote, citation and ranking in the SOURCE.
   - List the same in the REWRITE.
   - Compare the two lists. Anything present in one and missing from the other is an error, whether
     it was dropped or invented. Restore or remove it before continuing.
   - Then ask: **"What still sounds AI-generated?"**

   Asking yourself "did I change anything?" after the fact reliably answers no; two lists side by
   side answer honestly. In Embedded and File mode nobody sees intermediate output, so this is the
   only check there is.
