...
- **2.12.0** - Scoped the seven English-only rules and named the two that damage other languages:
  ...
  outlets while the skill's own rules call a lost claim an error. Made claim preservation a list
  comparison instead of a self-directed question, since Embedded and File mode show nobody the
  intermediate output.
